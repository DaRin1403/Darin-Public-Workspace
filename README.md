...
> 立绘角色：**月见八千代**，出自 Netflix 原创动画电影《超时空辉夜姬！》（2026，山下清悟执导）——虚拟偶像，真实身份是「辉夜姬」的意识数据化。
...
- **对话工具卡/网页卡**：蓝色液态玻璃两段式（标题深蓝 + 正文冰蓝，与代码语言统一）
- **设置区全覆盖**：模型页 / 预设页 / 插件配置卡（浓粉标题 + 淡粉正文两段式）/ 插件列表（浅粉玻璃状态徽章，语义绿灰保留）
- **上边栏细节**：后台任务菜单粉玻璃、「创造模式」预设小片独立液态玻璃胶囊
- **「跳到最新」按钮**：A 类粉玻璃三态圆形浮钮（与侧栏按钮同款）
- **占位符浅灰**：输入框「给 Kaguya 发消息」浅灰弱化，不抢正文
...
双击 `一键卸载.ps1`（PowerShell 运行）：自动移除挂载行并删除皮肤包，重启 `dsh web` 即恢复官方界面。
...
- **v1.1.0（2026-08-20）**：交互定稿 + 对话卡蓝色玻璃。新增：工具提示深灰字、后台任务菜单粉玻璃、「创造模式」预设小片液态玻璃胶囊、插件配置卡两段式（浓粉标题 + 淡粉正文 + 悬停白字）、插件状态徽章浅粉玻璃、轨迹详情标题块/详情标签模板 D（悬停艳粉白字）、网页卡/工具卡 IO 框蓝色液态玻璃两段式、蓝色发送键悬停加深（黑 7%→20%）、「跳到最新」粉玻璃圆钮、占位符浅灰、左下立绘按 27″4K 下移 8mm（160px→109px）；配套文档新增 `交互模板.md`、`交互架构方案.md`、`介绍.md`。
