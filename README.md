...
**一句话**：这是给 DeepSeek Harness 网页界面准备的「皮肤」——像给手机换壁纸一样，把原来的白灰界面换成樱花粉 + 毛玻璃质感的二次元风格。

- **只换外观，不动功能**：所有按钮、菜单、输入框都还在原位，功能一模一样，只是变好看了；
- **像毛玻璃一样通透**：面板半透明、背后微微模糊，配上一整幅背景图和左下角的 Kaguya 立绘；
- **按钮有手感**：鼠标放上去会变深、按下去会「凹」一下，每个操作都有清楚反馈；
- **不瞎改颜色**：红色 = 出错、绿色 = 成功、蓝色 = 发送、黄色 = 警告，这些状态色保持原样，一眼看懂；
- **只影响浅色模式**：切到深色模式时界面和官方完全一样，装了这皮肤也不会弄坏什么；
- **装起来就一条命令**：Windows 或 Mac 各复制一行命令到终端，回车，重启一下就好了（见下方「快速开始」）。

> 下文若提到「13 面墙」「GLASS_CSS」「变量账本」这类词——那是给想自己动手改皮肤的人看的技术细节，**普通使用完全可以跳过**。
...
- **为什么说"只刷 3 面墙"**：官方界面把颜色拆成 13 组变量（像 13 面墙，每面管一类颜色）。本皮肤只覆盖其中 3 面（主色、侧栏底、整页背景），其余 10 面保持官方原样，再额外加一层玻璃样式（GLASS_CSS）。用官方 1/3 的改动面换来完整的视觉世界观与统一的三态交互；深色模式 100% 保持官方原样，升级、切换主题零副作用。
- **玻璃四件套**：半透明渐变底色 + `backdrop-filter` 模糊 + 白色亮边 + 上高光/下内影/外投影，全 UI 同一套配方。
- **变量账本**：所有粉色都定义在 `--kaguya-*` 一组变量里（常驻/悬停/按下三档），改一个变量全局变色——计划中的 v1.2「主题色切换」就靠它。
- 完整工程规范见 [`设计规范.md`](anime-theme/设计规范.md) 与 [`交互模板.md`](anime-theme/交互模板.md)。
